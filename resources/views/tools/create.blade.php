...
<div class="container mx-auto px-4 py-6">
    <div class="flex items-center justify-between mb-6">
        <h2 class="text-2xl font-semibold text-gray-900">Nueva Herramienta</h2>
    ...
    <div class="bg-white shadow rounded-lg">
        <form action="{{ route('tools.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6 p-6">
            @csrf
            ...
            <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                ...
                <div>
                    ...
                    <select name="proficiency_level" id="proficiency_level" 
                        ...
                        @for($i = 1; $i <= 5; $i++)
                            <option value="{{ $i }}" {{ old('proficiency_level') == $i ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <input type="file" name="icon" id="icon" accept="image/*"
                        class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                    <p class="mt-1 text-sm text-gray-500">PNG, JPG, JPEG o SVG (máx. 1MB)</p>
                    @error('icon')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
